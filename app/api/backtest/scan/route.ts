...
function buildCurrentSummarySentence(params: {
  ...
  const regimeLabel: Record<string, string> = {
    "RED / EJECTED": "confirmed downtrend with kill-leverage signal active — price is below the Weekly 21 EMA, triggering the Master Eject condition. This means all leveraged exposure (TSLL, calls) should be off the table until price reclaims the Weekly 21 EMA. Stock-only, minimum size, or flat.",
  ...
  if ((mode.includes("RED") && avoidActiveCount > 0) || avoidActiveCount >= 2) {
    ...
    const dailyImproving = indicators?.bx_daily_state === "HL" || indicators?.bx_daily_state === "HH";
    if (dailyImproving && indicators?.bx_weekly_state === "LL") {
      stance = "DEFENSIVE WAIT. The daily structure is improving, but the weekly structure is still broken and price remains below the Weekly 21 EMA. That means early stabilization is happening inside a still-bearish higher-timeframe regime. Stay flat on new positions or keep size minimal until the weekly damage is repaired.\n\nWhat would change this stance: Weekly BX Trender improving out of LL, or price reclaiming the Weekly 21 EMA with daily BX staying constructive. Daily strength alone is not enough yet — the higher timeframe still controls the tape.\n\nIf forced to enter here: Follow current mode caps. In RED mode: 5% max exposure, stock only. In RED/EJECTED: no new positions. No leveraged ETFs or options — the higher timeframe trend is still structurally bearish. Risk spectrum: Stock (small size per mode cap) → TSLL/leveraged ETF (avoid) → Calls (avoid entirely).";
    } else {
      stance = "DEFENSIVE WAIT. The trend is bearish but no specific caution setups have triggered, meaning the downtrend may be mature rather than accelerating. Stay flat on new positions. The re-entry trigger is the higher timeframe structure repairing — ideally Weekly BX Trender improving and price reclaiming the Weekly 21 EMA.\n\nWhat would change this stance: Weekly BX improving from LL, or RSI dropping below 30 (which would activate oversold bounce setups). SMI crossing back above -20 would also be an early improvement signal, but the Weekly 21 EMA remains the real line in the sand.\n\nIf forced to enter here: Follow current mode caps. In RED mode: 5% max exposure, stock only. In RED/EJECTED: no new positions. No leveraged ETFs or options — the trend is still structurally bearish. Risk spectrum: Stock (small size per mode cap) → TSLL/leveraged ETF (avoid) → Calls (avoid entirely).";
    }
