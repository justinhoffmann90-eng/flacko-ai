...
export default async function TickerDateReportPage({ params }: Props) {
  const { ticker: rawTicker, date } = await params;
  ...
  if (!devBypass) {
    ...
    if (!user) {
      redirect(`/login?next=/report/${ticker.toLowerCase()}/${date}`);
    }
    userId = user.id;
  }
  ...
  // Fetch specific date report for this ticker
  ...
  if (!report) {
    return (
      <>
        <Header title={`${ticker} Report — ${date}`} />
        <main className="px-4 py-6 max-w-2xl mx-auto space-y-4">
          <Card>
            <CardContent className="pt-8 pb-8 text-center space-y-3">
              ...
              <p className="text-lg font-medium">No {ticker} report for {date}</p>
              <p className="text-sm text-muted-foreground max-w-sm mx-auto">
                This report may not have been published yet, or this date was not a trading day.
              </p>
              <a
                href={`/report/${ticker.toLowerCase()}`}
                className="inline-flex items-center justify-center rounded-md text-sm font-medium bg-secondary text-secondary-foreground shadow-sm h-10 px-6 hover:bg-secondary/80 transition-colors"
              >
                View Latest Report
              </a>
            </CardContent>
