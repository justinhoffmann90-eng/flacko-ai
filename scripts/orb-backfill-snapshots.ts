/**
 * Backfill orb_daily_snapshots from historical data.
 * Produces one row per setup per day — the "atoms" for Orb Score backtesting.
 * Reuses the same indicator math + evaluation logic as orb-backtest.ts.
 */
import { readFileSync } from "node:fs";
import { createClient } from "@supabase/supabase-js";
import { evaluateAllSetups, type Indicators, type PreviousState } from "../lib/orb/evaluate-setups";

type Quote = { date: string; open: number; high: number; low: number; close: number; volume?: number };
...
function mapSetupId(id: string) { return ID_MAP[id] || id; }
...
function rsi(data: number[], period = 14): number[] {
  ...
  return avgGain.map((g, i) => { if (avgLoss[i] === 0) return 100; return 100 - 100 / (1 + g / avgLoss[i]); });
}

function smi(close: number[], high: number[], low: number[], kLength = 10, dLength = 3, smooth = 3) {
  const n = close.length;
  const ll: number[] = [], hh: number[] = [];
  for (let i = 0; i < n; i++) {
  ...
  const d1 = ema(relDiff, dLength), d2 = ema(d1, smooth);
  const r1 = ema(relRange, dLength), r2 = ema(r1, smooth);
...
function getEnv() {
  ...
  if (!url || !key) throw new Error("Missing env vars in .env.local");
  return { url, key };
}
...
type SnapshotRow = {
  ...
  conditions_met: Record<string, boolean> | null;
  reason: string | null;
  gauge_current_value: number | null;
  gauge_progress_pct: number | null;
};

async function main() {
  const quotes = await fetchTslaDaily();
  if (quotes.length < 260) throw new Error(`Not enough bars: ${quotes.length}`);
  console.log(`Fetched ${quotes.length} daily bars (${quotes[0].date} to ${quotes[quotes.length - 1].date})`);
  ...
  for (let i = 1; i <= quotes.length; i++) {
    ...
    if ((!isLast && currDay < prevDay) || isLast) {
  ...
  const activeDays = new Map<string, number>(); // track active_day per setup
  const entryPrices = new Map<string, number>(); // track entry_price per setup
  ...
  for (let i = 250; i < quotes.length; i++) {
    ...
    let consDown = 0, consUp = 0;
    for (let j = i; j > 0; j--) { if (closes[j] < closes[j - 1]) consDown++; else break; }
    for (let j = i; j > 0; j--) { if (closes[j] > closes[j - 1]) consUp++; else break; }
    ...
    const indicators: Indicators = {
      ...
      stabilization_days: (() => { let d = 0; for (let j = i; j > 0; j--) { if (lows[j] >= lows[j - 1]) d++; else break; } return d; })(),
      ...
      daily_hh_streak: (() => { let s = 0; for (let j = i; j >= 2; j--) { if (classifyBxState(bx[j], bx[j - 1]) === "HH") s++; else break; } return s; })(),
      ema9_slope_5d: i >= 5 ? ((ema9[i] - ema9[i - 5]) / ema9[i - 5]) * 100 : 0,
      days_below_ema9: (() => { let d = 0; for (let j = i; j > 0; j--) { if (closes[j] < ema9[j]) d++; else break; } return d; })(),
      was_full_bull_5d: (() => { for (let j = Math.max(0, i - 4); j <= i; j++) { if (closes[j] > ema9[j] && ema9[j] > ema21[j]) return true; } return false; })(),
    };
    ...
    for (const result of results) {
      ...
      const prevStatus = before.get(setupId)?.status || "inactive";
      const newStatus = result.is_active ? "active" : result.is_watching ? "watching" : "inactive";

      // Track active_day and entry_price
      if (newStatus === "active" && prevStatus !== "active") {
        activeDays.set(setupId, 1);
        entryPrices.set(setupId, closes[i]);
      } else if (newStatus === "active") {
        activeDays.set(setupId, (activeDays.get(setupId) || 0) + 1);
      } else {
        activeDays.delete(setupId);
        entryPrices.delete(setupId);
      }
      ...
      allSnapshots.push({
        ...
        setup_id: setupId,
        status: newStatus,
        entry_price: newStatus === "active" ? (entryPrices.get(setupId) ?? null) : null,
        active_day: newStatus === "active" ? (activeDays.get(setupId) ?? null) : null,
        conditions_met: result.conditions_met ?? null,
        reason: result.reason || null,
        gauge_current_value: result.gauge_current_value ?? null,
        gauge_progress_pct: null, // Could compute but not critical for score backtest
      });
  ...
  console.log(`\nGenerated ${allSnapshots.length} snapshot rows`);
  const uniqueDates = new Set(allSnapshots.map(s => s.date));
  console.log(`Covering ${uniqueDates.size} trading days × 15 setups`);

  // Status distribution
  const statusCounts = { active: 0, watching: 0, inactive: 0 };
  for (const s of allSnapshots) statusCounts[s.status as keyof typeof statusCounts]++;
  console.log(`Status distribution: active=${statusCounts.active}, watching=${statusCounts.watching}, inactive=${statusCounts.inactive}`);

  // Write to DB
  const { url, key } = getEnv();
  const supabase = createClient(url, key);

  // Clear existing snapshots (backfill replaces all)
  console.log("\nClearing existing snapshots...");
  const { error: delErr } = await supabase.from("orb_daily_snapshots").delete().neq("id", 0);
  if (delErr) throw delErr;

  console.log("Writing snapshots in chunks...");
  let written = 0;
  for (const c of chunk(allSnapshots, 500)) {
    const { error } = await supabase.from("orb_daily_snapshots").upsert(c, { onConflict: "date,setup_id" });
    if (error) throw error;
    written += c.length;
    process.stdout.write(`\r  ${written}/${allSnapshots.length} rows written`);
  }
  console.log("\n\nBackfill complete!");

  // Quick validation: print active count by date for last 5 dates
  const lastDates = Array.from(uniqueDates).sort().slice(-5);
  console.log("\nLast 5 days active setup counts:");
  for (const d of lastDates) {
    const dayRows = allSnapshots.filter(s => s.date === d);
    const activeCount = dayRows.filter(s => s.status === "active").length;
    const watchingCount = dayRows.filter(s => s.status === "watching").length;
    const activeNames = dayRows.filter(s => s.status === "active").map(s => s.setup_id).join(", ");
    console.log(`  ${d}: ${activeCount} active, ${watchingCount} watching${activeNames ? ` [${activeNames}]` : ""}`);
  }
}
