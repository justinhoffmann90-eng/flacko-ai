...
export function DiscordOnboarding({ isDiscordLinked }: DiscordOnboardingProps) {
  ...
  // Don't show if not mounted or already linked
  if (!mounted || isDiscordLinked) {
    return null;
  }
  ...
  return (
    <Card className="relative p-4 bg-[#5865F2]/10 border-[#5865F2]/30">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-sm">Connect Discord to unlock alerts & community</h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            One click to join the server with the <span className="text-[#5865F2] font-medium">Subscriber</span> role — real-time price alerts, research channels, and Taylor&apos;s live paper trades.
          </p>
          <Button
            size="sm"
